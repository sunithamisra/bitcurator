...
if __name__=="__main__":
    ...
    if args.pdf_report:
        ...
        print("1: Generate a report of first x number of files")
        print("2: Generate a report of last x number of files")
         
        display_option1 = int(input ("option: "))

        print("Please input the number of files you want the report on")
        display_numfiles = int(input("enter number of files:"))

        ##print("D: ", display_option1)

        if display_option1==1:
            print("Generating report of first %d files" % display_numfiles)
        elif display_option1==2:
            print("Generating report of last %s files" % display_numfiles, display_option1)
        else:
            print("Wrong Option: Defaulting to 1")
            display_option1 = 1

        print("Option %s selected " %  display_option1)


        report = generate_report.PdfReport(args.outdir)
        report.process_generate_report(args)
        ...
        if args.fiwalk_txtfile:
            ...
            ##print("Using Fiwalk TXT file ", fiwalk_txtfile)
            #fiwalk.fiwalk_using_sax(xmlfile=open(xmlfile,'rb'),callback=process)
            report_fi = generate_report.DirReport(args.fiwalk_txtfile)
            ##print("CALling procesS_Generate_fwalk  ", fiwalk_txtfile)

            report_fi.process_generate_report_fiwalk(args, display_option1, display_numfiles)
        exit(1)
