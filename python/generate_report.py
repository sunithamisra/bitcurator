...
Note-1: It uses the python-3 port of FPself.DF software from 
...
class PDF(FPDF):
        ...
        def make_table_delfiles(self, header):
            ...
            for i in range(0, len(DirReport.myDictList)):
                if DirReport.myDictList[i]['unalloc']:
                    num_deleted_files+=1
                    #print("DDDDDDD: Deleted File: ", num_deleted_files, DirReport.myDictList[i]['filename'])    
                    self.cell(w[0],6,str(num_deleted_files),'LR',0,'L',fill)
                    ##self.cell(w[1],6,DirReport.myDictList[i]['filename'],'LR',0,'L',fill)
                    #mystr = (DirReport.myDictList[i]['filename'])[:-10]
                    mystr = (DirReport.myDictList[i]['filename'])
                    #mystr = "sunitha" + str(num_deleted_files)
                    self.cell(w[1],6,mystr,'LR',0,'L',fill)
                    self.ln()
                    fill=not fill
        ...
        def make_table(self, header, display_option1, display_numfiles):
                ...
                #self.chapter_title(1,'File List')
                ##self.cell(40, 7, "File List",1)
                ...
                if 1==int(display_option1):
                    # Report first x files
                    range1 = 0
                    range2 = range1 + int(display_numfiles)
                    print("Reporting FIRST %s files " % display_numfiles, range1, range2 )
                else:
                    range1 = DirReport.array_ind - int(display_numfiles) 
                    range2 = DirReport.array_ind - 1
                    print("Reporting LAST %s files" % display_numfiles, range1, range2)
                #for i in range(0, DirReport.array_ind):
...
def create_report_file(input_file, annotated_file):
    ...
    #SSS: Table Name
    ##PDF.cell(40, 7, annotated_file, 0, 1, 'L', 1)
    ...
    #pdf_file = annotated_file[-len('annotated_'):-4] + '.pdf'
    pdf_file = annotated_file[10:-3] + 'pdf'
    ...
    def process_generate_report(self, fn):
        ...
        for annotated_file in self.files:
            ...
            for line in ifd:
                ...
                # Add the line info to the feature report
                ####add_feature_report(pdf_file, line)
...
def fw_get_file_info(line1):
    ##print("fw_get_file_info: line10: line11:", line1[0], line1[1])
    #if line1[0] in image_fileinfo[0]:
        #print ("line10 is skip_lines0", image_fileinfo[0])
        #image_fileinfo_val[0] = line1[1]
    #elif line1[0] in image_fileinfo[1]:
    ...
    for i in range(0, (len(image_fileinfo)-1)):
        if line1[0] in image_fileinfo[i]:
            ...
            break
    ## else:
        ## print("DEBUG: No MATCH", line1[0], line1[1])
...
def make_dict(linenum, line):
    ...
    if not line.strip():
        ...
        return
    line1 = re.split(":", line)
    #if line1[0] == 'md5' or line[0] == 'sha1':
    if 'md5' in line1[0] or 'sha1' in line1[0]:
        ##print("DEBUG: SKIPPING MD5 and SHA1: line1:", line1[0])
        return
    if line1[0] in image_fileinfo:
        ##print("DEBUG:FILE INFO ", line1[0])
        fw_get_file_info(line1) 
        return
    #if re.match("filename: ",line):
        ###print(" HIT First fileinfo ")
        #DirReport.dont_start_yet = False
        ##print("line[1] is filename? ", line1[1])
        ##print(" DirReport.arrayElementNum =",  DirReport.arrayElementNum)
        ##DirReport.myDictList.append(line[0])
    #elif DirReport.dont_start_yet:
        #return
    ##if line1[0] in 'seq': #Last element
    ...
    if line1[0] in 'filename': #Next sub-array 
        ...
        if DirReport.is_first_file==True:
            ##print("TOGGLING the is_first_file flag ")
            DirReport.is_first_file = False
        else:
            ## print("NOT first file anymore is_first_file flag is false ")
            ...
            ## print("MAKE_DICT: ALLOC:, UNALLOC: ", DirReport.dict_array[5],DirReport.dict_val[5], DirReport.dict_array[5],DirReport.dict_val[6]) 

        ## print("D: MYDICTLIST", DirReport.myDictList)
    ## print("D: LINE1[0] = ", line1[0])
    for ind in range (0, (len(DirReport.dict_array)-1)):
        ## print("DICT_ARRAY[ind]=", ind, DirReport.dict_array[ind])
...
def get_file_info(line):
    ...
    if re.match("name_type: ",line):
        ...
        if "r" in line1[1]:
            ...
            ##print("FILES: ",DirReport.numfiles)
        elif "d" in line1[1]:
            DirReport.dirs = DirReport.dirs + 1
            ##print("DIRS: ",DirReport.dirs)
...
def write_file_details(of):
    ...
    for i in range(0, DirReport.array_ind-1):
      #for j in range(0, 21):
        ...
        of.write(b"\n")
        #of.write(DirReport.dict_array[i]["filename"])
        #of.write(bytes(DirReport.dict_array[i][j], 'UTF-8'))

#Make a plot with figure size (x,y), range 'range', etc
def plotGraph(X,Y, curr_range, val_array, page_num, rstr):
    ### Set all plotting arrangments here ###
    ## print ("D: val_array: ", val_array)
    fig = plt.figure(figsize=(X,Y))
    plot(range(curr_range), val_array, rstr)
    title('File Format Frequency Distribution')
    plt.xlabel('File Formats')
    plt.ylabel('Frequency')
    
    return fig

def bc_draw_histogram_fileformat():
    # Make new plots, 3x3 and 4x4 size
    ##samples = numpy.array(DirReport.dictFileFmtVal)
    ###fig = plotGraph(3,3,7,[3,1,4,1,5,9,2],'1','r-o')
    fig = plotGraph(10,10,16,DirReport.dictFileFmtList,'1','r-o')
    #fig2 = plotGraph(4,4,7,[2,3,4,1,8,3,4],'2','r-o')
    ##data_to_plot = numpy.array([1, 1, 1, 3, 2, 5, 1, 10, 10, 8])
    ##samples = numpy.array(DirReport.dictFileFmtVal)
    ##samples = numpy.array(data_to_plot)

    #Initialize the pdf
    pp = PdfPages('bc_plot.pdf')
    ...
    ## os.system("evince ./bc_plot.pdf")


'''
    # Create the PdfPages object to which we will save the pages:
    pdf = PdfPages('bc_feat_histogram.pdf')
...
'''
    print("HISTOGRAM")
    ##data_to_plot = DirReport.dictFileFmtVal
    data_to_plot = numpy.array([1, 1, 1, 3, 2, 5, 1, 10, 10, 8])
    ##samples = numpy.array(DirReport.dictFileFmtVal)
    samples = numpy.array(data_to_plot)
    N = 20 # FIXME: Need to find the appropriate number
    #plt.hist(data_to_plot)
    n, bins, patches  = plt.hist( samples, N, facecolor="magenta", 
                                  range=[1,N], normed=True )
    plt.title("Bitcurator File Format Histogram")
    plt.xlabel("Frequency")
    plt.ylabel("File Formats")

    #Initialize the pdf
    pp = PdfPages('hist1.pdf')
...
def fw_print_fileinfo(pdf):
    ...
    pdf.underline = 0
    for i in range(0, (len(image_fileinfo)-1)):
        ##newstring = image_fileinfo[i] + ': ' + image_fileinfo_val[i]
...
class DirReport:
    ...
    def process_generate_report_fiwalk(self, fn, display_option1, display_numfiles):
        print("DEBUG: ", fn.outdir, fn, display_option1, display_numfiles)
        ...
        for i in range(0, (len(self.dictFileFmtArray)-1)):
            ...
            self.dictFileFmtList[i] = self.dictFileFmtVal[i]
        bc_draw_histogram_fileformat()

        # Table header
        header_stat = ['Statistics','']
        header_be = ['Bulk Extractor Report Files','Feature Instances','FLTF','FIUF','FICR']
        #header_files = ['filename','partition','ID','DIR','size','Deleted','used?','meta_type','mode','nlink']
        ...
        pdf.set_font('Arial','',10)
        pdf.add_page()
        pdf.make_table(header_files, display_option1, display_numfiles)
        pdf.output('FiwalkReport.pdf','F')
        bc_addToReportFileList('FiwalkReport.pdf')
        ...
        pdf.output("FiwalkDeletedFiles.pdf",'F')
        bc_addToReportFileList('FiwalkDeletedFiles.pdf')
        ...
        pdf.output('BeReport.pdf','F')
        bc_addToReportFileList('BeReport.pdf')
        #os.system("evince ./BeReport.pdf")
