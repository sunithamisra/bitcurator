...
class HTML2FPDF(HTMLParser):
    ...
    def handle_data(self, txt):
        if self.td is not None:  # drawing a table?
            ...
            if not self.th:
                ...
                border = border or 'B'
                align = 'C'
            bgcolor = hex2dec(self.td.get(
                'bgcolor', self.tr.get('bgcolor', '')))
            ...
            if DEBUG:
                print("cell", txt, "*")
            self.pdf.cell(0, self.h, txt, 0, 1, self.align[0]
                .upper(), self.href)
        else:
            txt = txt.replace("\n", " ")
            if self.href:
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'font':
            ...
            self.font_stack.append(
                (self.font_face, self.font_size, self.color))
            if 'color' in attrs:
    ...
    def put_line(self):
        ...
        ), self.pdf.get_x() + 187, self.pdf.get_y())
        self.pdf.ln(3)
