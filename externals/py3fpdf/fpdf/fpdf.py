...
class FPDF(object):
    ...
    def __init__(self, orientation='P', unit='mm', format='A4'):
        ...
        self.buffer = b''                 # buffer holding in-memory PDF
        self.pages = {}                   # dictionary containing pages
        self.orientation_changes = {}
            # dictionary indicating orientation changes
    ...
    def set_display_mode(self, zoom, layout='continuous'):
        ...
        if layout == 'single' or layout == 'continuous' or \
           layout == 'two' or layout == 'default':
    ...
    def set_draw_color(self, r, g=-1, b=-1):
        ...
        if((r == 0 and g == 0 and b == 0) or g == -1):
            ...
            self.draw_color = sprintf('%.3f %.3f %.3f RG', r /
                                      255.0, g / 255.0, b / 255.0)
        if(self.page > 0):
    ...
    def set_fill_color(self, r, g=-1, b=-1):
        ...
        if((r == 0 and g == 0 and b == 0) or g == -1):
            ...
            self.fill_color = sprintf('%.3f %.3f %.3f rg', r /
                                      255.0, g / 255.0, b / 255.0)
    ...
    def set_text_color(self, r, g=-1, b=-1):
        ...
        if((r == 0 and g == 0 and b == 0) or g == -1):
            ...
            self.text_color = sprintf('%.3f %.3f %.3f rg', r /
                                      255.0, g / 255.0, b / 255.0)
        self.color_flag = (self.fill_color != self.text_color)
    ...
    def line(self, x1, y1, x2, y2):
        "Draw a line"
        self._out(sprintf('%.2f %.2f m %.2f %.2f l S', x1 * self.k, (self.h -
                                                                     y1) * self.k, x2 * self.k, (self.h - y2) * self.k))
    ...
    def add_font(self, family, style='', fname='', uni=False):
        ...
        if uni:
            ...
            if os.path.exists(unifilename):
                ...
                font_dict = {
                    'name': re.sub('[ ()]', '', ttf.fullName.decode()),
                    'type': 'TTF',
                    'desc': desc,
                ...
                with open(unifilename, "w") as file_:
                    try:
                        pickle.dump(font_dict, file_)
                    except IOError as e:
    ...
    def set_font(self, family, style='', size=0):
        ...
        if self.font_family == family and self.font_style == style  and \
        ...
        if self.page > 0:
            self._out(sprintf('BT /F%d %.2f Tf ET',
                              self.current_font['i'], self.font_size_pt))
    ...
    def link(self, x, y, w, h, link):
        ...
        self.page_links[self.page] += [(x * self.k, self.h_pt - y *
                                        self.k, w * self.k, h * self.k, link), ]
    ...
    def cell(self, w, h=0, txt='', border=0, ln=0, align='', fill=0, link=''):
        ...
        if fill == 1 or border == 1:
            if fill == 1:
                ...
                op = 'S'
            s = sprintf('%.2f %.2f %.2f %.2f re %s ', self.x *
                        k, (self.h - self.y) * k, w * k, -h * k, op)
    ...
    def image(self, name, x=None, y=None, w=0, h=0, type='', link=''):
        ...
        # Flowing mode
        if y is None:
            if self.y + h > self.page_break_trigger and not \
                   self.in_footer and self.accept_page_break():
                #Automatic page break
                x = self.x
                self.add_page(self.cur_orientation)
                self.x = x
...
#            self.error("Don\'t alter the locale before including class file");
        ...
        for n in range(1, nb + 1):
            ...
            if self.page_links and n in self.page_links:
                ...
                for pl in self.page_links[n]:
                    rect = sprintf('%.2f %.2f %.2f %.2f', pl[0], pl[
                        1], pl[0] + pl[2], pl[1] - pl[3])
                    annots += '<</Type /Annot /Subtype /Link /Rect [' + \
                        rect + '] /Border [0 0 0] '
                    if(isinstance(pl[4], str)):
                        annots += '/A <</S /URI /URI ' + self._textstring(
                            pl[4]) + '>>>>'
                    else:
                        l = self.links[pl[4]]
    ...
    def _putfonts(self):
        ...
        for name, info in self.font_files.items():
            if 'type' in info and info['type'] != 'TTF':
                ...
                # if the font dont exists this is going to blow.
                with  open(self._getfontpath() + name, 'rb', 1) as font_file:
        ...
        for k, font in self.fonts.items():
            ...
            if(type == 'core'):
                ...
                if (font['desc'].get('MissingWidth')):
                    self._out('/DW %d' % font['desc']['MissingWidth'])._putTTfontwidths(font, ttf.maxUni)
                ...
                for cc, glyph in list(codeToGlyph.items()):
    ...
    def _putimage(self, info):
        if 'data' in info:
            ...
            if info['cs'] == 'Indexed':
                # the int() call in the next method call is
                # required, because of the new non trucated
                # divition in python 3.
                # :S I spend like... 5 hours with this bug.
                # if the number is float then gets writter as NUM.
                # istead of just NUM, the fucking DOT.
                self._out('/ColorSpace [/Indexed /DeviceRGB ' + \
                          str(int(len(info['pal']) / 3 - 1)) + ' ' +  \
                          str(self.n + 1) + ' 0 R]')
            else:
    ...
    def _putinfo(self):
        self._out('/Producer ' + self._textstring('PyFPDF ' +
                                                  FPDF_VERSION + ' http://pyfpdf.googlecode.com/'))
    ...
    def _putcatalog(self):
        ...
        if(self.zoom_mode == 'fullpage'):
            ...
            self._out('/OpenAction [3 0 R /XYZ null null ' + (
                self.zoom_mode / 100) + ']')
    ...
    def _parsepng(self, name):
        ...
        png_sig = b''.join((b'\x89', b'PNG', b'\r', b'\n', b'\x1a', b'\n'))
        #Check signature
        if f.read(8) != png_sig:
        ...
        w = self._freadint(f)
        h = self._freadint(f)
        bpc = ord(f.read(1)) # we use ord, because we whant to use the integer
    ...
    def _freadint(self, f):
        ...
        data_ = f.read(4)
        try:
            return struct.unpack('>HH', data_)[1]
        except:
            return None
