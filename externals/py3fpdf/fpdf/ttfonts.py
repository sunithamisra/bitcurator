...
class TTFontFile:
    ...
    def getMetrics(self, file):
        ...
        if version not in (0x00010000, 0x74727565):
            die("Not a TrueType font: version=%s" % version)
    ...
    def read_tag(self):
        self._pos += 4
        return self.fh.read(4).decode('ascii')
...
############################################/
        ...
        for originalGlyphIdx, uni in subsetglyphs:
            ...
            if (glyphLen > 2 and (up & (1 << 15))):     # If number of contours <= -1 i.e. composiste glyph
                ...
                while (flags & GF_MORE):
                    ...
                    try:
                        data = self._set_ushort(data, pos_in_glyph +
                                                2, glyphSet[glyphIdx])
                    except KeyError:
                        data = 0
                ...
                maxComponentElements = max(
                    maxComponentElements, nComponentElements)
    ...
    def getHMTX(self, numberOfHMetrics, numGlyphs, glyphToChar, scale):
        ...
        if ((numberOfHMetrics * 4) < self.maxStrLenRead):
            ...
            arr = unpack(">" + "H" * (int(len(data) / 2)), data)
        else:
            self.seek(start)
    ...
    def endTTFile(self, stm):
        ...
        # Header
        if (_TTF_MAC_HEADER):
            stm += (pack(">LHHHH", 0x74727565, numTables,
