...
class Template:
    def __init__(self, infile=None, elements=None, format='A4', orientation='portrait',
        ...
        self.texts = {}
        pdf = self.pdf = FPDF(
            format=format, orientation=orientation, unit="mm")
        pdf.set_title(title)
    ...
    def text(self, pdf, x1=0, y1=0, x2=0, y2=0, text='', font="arial", size=10,
        ...
        if text:
            ...
            if underline:
                style += "U"
            align = {'L': 'L', 'R': 'R', 'I': 'L', 'D': 'R',
                     'C': 'C', '': ''}.get(align)  # D/I in spanish
            ...
            if multiline is None:
                ...
                pdf.multi_cell(w=x2 - x1, h=y2 - y1,
                               txt=text, border=0, align=align)
            else:
...
if __name__ == "__main__":
    ...
    for i in range(1, 30):
        ...
        code = "%s%s%02d" % (chr(
            random.randint(65, 90)), chr(random.randint(65, 90)), i)
    ...
    for it in items:
        ...
        for ds in f.split_multicell(it['ds'], 'item_description01'):
            ...
            li_items.append(dict(code=code, ds=ds, qty=qty,
                                 unit=unit, price=None, amount=None))
